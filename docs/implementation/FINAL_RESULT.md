...
| Legacy wizard | `/legacy` (+ `USE_LEGACY_UI`) |
...
| Dashboard | Manager stats API + Active Sending card |
...
2. **Dashboard stats parity**: React adapter uses manager statistics endpoints + Active Sending; not a pixel-perfect port of every legacy `statistics.js` filter/export control.
