...
describe('useCampaignLaunchValidation', () => {
  ...
  it('retry() re-fetches even when the signature is unchanged and cached data exists', async () => {
    vi.mocked(campaignsApi.validate).mockResolvedValue({
      ok: true,
      errors: [],
      warnings: ['cached'],
  ...
  it('exposes the raw thrown error instead of a collapsed string', async () => {
    const apiError = new ApiError(500, { message: 'Сервис проверки недоступен', hint: 'Повторите позже' });
    vi.mocked(campaignsApi.validate).mockRejectedValue(apiError);
    const queryClient = new QueryClient({ defaultOptions: { queries: { retry: false } } });
    const flush = vi.fn().mockResolvedValue(undefined);
