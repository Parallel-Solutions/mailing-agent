...
def _inflect_municipality_tail(tail: str, target_case: str) -> tuple[str, str, tuple[str, ...]]:
    ...
    council_result = _inflect_council_tail(tail, target_case)
    if council_result:
        return council_result, "component_rule", ()
...
def _inflect_council_tail(tail: str, target_case: str) -> str | None:
    parts = _normalize_spaces(tail).split()
    if len(parts) < 2:
        return None
    ...
    noun_forms = {
        "сельсовет": {
            "genitive": "сельсовета",
            "project_genitive": "сельсовета",
            "prepositional": "сельсовете",
        },
        "поссовет": {
            "genitive": "поссовета",
            "project_genitive": "поссовета",
            "prepositional": "поссовете",
        },
    }
    noun = _normalize_marker(parts[-1])
    forms = noun_forms.get(noun)
    if not forms:
        return None

    grammemes = _CASE_GRAMMEMES[target_case]
    head = " ".join(_inflect_hyphenated_tail_word(part, grammemes) for part in parts[:-1])
    return f"{head} {forms[target_case]}".strip()


def _inflect_hyphenated_tail_word(word: str, grammemes: set[str]) -> str:
    if "-" not in word:
        inflected = _simple_word_inflect(word, grammemes)
        return _restore_word_case(word, inflected)

    parts = word.split("-")
    last = parts[-1]
    inflected_last = _simple_word_inflect(last, grammemes)
    parts[-1] = _restore_word_case(last, inflected_last)
    return "-".join(parts)
...
def _phrase_inflect(phrase: str, grammemes: set[str]) -> str:
    ...
    for word in normalized.split():
        ...
        if not clean:
            ...
            continue
        if "-" in clean:
            inflected = _inflect_hyphenated_tail_word(clean, grammemes)
        else:
