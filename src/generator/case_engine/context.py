...
def build_context_sentence(field: str, context: dict) -> str:
    ...
    mapping = {
        ...
        "MUN_NAME_2": f"по {work_title} [SLOT]",
        "MUN_NAME_3": "работы выполняются в [SLOT]",
        "MUN_R_NAME_1": f"по {work_title} {context.get('MUN_NAME_2', '')} [SLOT] {context.get('SUB_RF_1', '')}",
        "SUB_RF_1": f"по {work_title} {context.get('MUN_NAME_2', '')} {context.get('MUN_R_NAME_1', '')} [SLOT]",
...
def build_slot_instruction(field: str) -> str:
    mapping = {
        ...
        "MUN_NAME_2": "Проверь название муниципального образования в проектной фразе для выбранного вида работ.",
