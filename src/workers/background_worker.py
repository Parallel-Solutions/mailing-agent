...
def _run_sender(kwargs: dict[str, Any]) -> None:
    ...
    try:
        run_sender(
            ...
            smtp_mailbox_id=kwargs.get("smtp_mailbox_id"),
...
def run_payload(payload: dict[str, Any]) -> None:
    ...
    if task == "chain_followup":
        ...
        return
    if task == "recipient_resend":
        from src.campaigns.recipient_resend_service import run_recipient_resend

        run_recipient_resend(kwargs)
        return
