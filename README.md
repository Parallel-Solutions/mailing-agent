...
Gotenberg не публикуется наружу и доступен только контейнеру приложения по адресу `http://gotenberg:3000`. В Docker-сценарии PDF-конвертация настроена через Gotenberg, резервный LibreOffice-backend по умолчанию отключён.
