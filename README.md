...
API-ключ доступа RuSender задаётся один раз в серверной переменной
`RUSENDER_API_KEY` и не сохраняется в подключениях. Для каждого подключения
RuSender пользователь указывает только числовой ID активного ключа отправки и
подтверждённый email. Для ключей формата `rs_ck_v1_...` запрос отправляется с
`Authorization: Bearer` на `/external-mails/send/{key_id}`; API-ключ должен
иметь право `external_mail.send`.
...
- `EMAIL_VALIDATION_MODE` — режим проверки адресов. По умолчанию `domain`: синтаксис и DNS/MX проверяются локально. Значение `smtpbz` дополнительно запускает справочную проверку SMTP.BZ, но её недоступность или неоднозначный ответ не блокируют отправку.

- `SMTPBZ_API_KEY` — необязательный API-ключ SMTP.BZ. Результаты провайдера отображаются как справочные и не исключают адреса из отправки; `validDeliver=false`, timeout, ошибка квоты и неизвестный ответ считаются неподтверждённым результатом. `SMTPBZ_FAIL_OPEN` сохранён только для совместимости и больше не меняет поведение.
