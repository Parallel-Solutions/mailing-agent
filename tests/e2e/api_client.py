...
from tests.e2e.config import E2EConfig, TRANSPORT
...
class E2EApiClient:
    ...
    def health_rusender_webhook(self) -> dict[str, Any]:
        return self._json(self._request("GET", "/api/webhooks/rusender"))
    ...
    def documents_start(self, job_id: str, *, document_mode: str, work_type: str, mode: str = "fast") -> dict[str, Any]:
        payload = self._json(
            self._request(
                "POST",
                "/api/documents/start",
    ...
    def wait_sender(self, job_id: str, *, expect_dry_run: bool) -> dict[str, Any]:
        ...
        while time.monotonic() < deadline:
            ...
            time.sleep(2.0)
        raise E2EApiError(f"Timed out waiting for sender (job={job_id}, dry_run={expect_dry_run}). Last={last}")
